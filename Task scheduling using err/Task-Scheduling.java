...
public class Taskscheduling {
	...
	private static List<Cloudlet> createCloudlet(int userId, int cloudlets){
		// Creates a container to store Cloudlets(tasks)
		LinkedList<Cloudlet> list = new LinkedList<Cloudlet>();

		//cloudlet(task) parameters
		long length = 5000;
		long fileSize = 300;
		long outputSize = 300;
		int pesNumber = 1;
		...
		for(int i=0;i<cloudlets;i++){
			...
			// setting the owner of these cloudlets(tasks)
	...
	private static void printCloudletList(List<Cloudlet> list) {
		...
		for(int i=0;i<totalCloudlets;i++)
		{
			...
			{
				if(smallestCloudlet.getActualCPUTime() < checkCloudlet.getActualCPUTime())
				{
		...
		timeQuantum = sortList.get(0).getActualCPUTime();
